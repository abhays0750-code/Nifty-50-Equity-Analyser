...
================================================================================
  40 Nifty stocks · 5-year data · Monthly move detection
...
def fetch_monthly(ticker: str) -> pd.DataFrame:
    """Returns monthly OHLC with pct change. Direction tagged dynamically."""
...
def tag_events(df_monthly: pd.DataFrame, threshold: float) -> pd.DataFrame:
    """Apply threshold to monthly data and tag spike/drop/flat."""
    m = df_monthly.copy()
    m["dir"] = m["pct"].apply(
        lambda x: "spike" if x >= threshold
        else ("drop" if x <= -threshold else "flat"))
    return m
...
# ══════════════════════════════════════════════════════════════════════════════
# AI LAYER  — date-specific news + concall link search
# ══════════════════════════════════════════════════════════════════════════════

def get_news_for_date(company: str, exact_date: str) -> dict:
    """
    Search news for the EXACT date of a price event.
    exact_date format: "19 Jan 2024"

    Returns dict with:
      - articles : list of {title, snippet, url}
      - concall  : {title, url} or None
    """
    if not SERPER_API_KEY:
        return {"articles": [], "concall": None}

    out, seen = [], set()
    concall_result = None

    # Build targeted date-specific queries
    queries = [
        f"{company} {exact_date} share price news",
        f"{company} {exact_date} quarterly results earnings",
        f"{company} {exact_date} announcement",
    ]
    concall_queries = [
        f"{company} concall transcript {exact_date[:7]}",   # e.g. "Jan 2024"
        f"{company} investor call earnings call {exact_date[-4:]}",
    ]
    ...
    for q in queries:
        try:
            r = requests.post(
                ...
                data=json.dumps({"q": q, "num": 3, "gl": "in"}),
                timeout=10)
            r.raise_for_status()
    ...
    # Concall link search — separate targeted search
    for q in concall_queries:
        try:
            r = requests.post(
                "https://google.serper.dev/search",
                headers={"X-API-KEY": SERPER_API_KEY,
                         "Content-Type": "application/json"},
                data=json.dumps({"q": q, "num": 5, "gl": "in"}),
                timeout=10)
            r.raise_for_status()
            for item in r.json().get("organic", []):
                url   = item.get("link", "")
                title = item.get("title", "")
                # Look for concall-specific domains and keywords
                concall_keywords = [
                    "concall", "investor call", "earnings call",
                    "conference call", "transcript", "bseindia.com",
                    "nseindia.com", "screener.in", "trendlyne"
                ]
                if any(kw in url.lower() or kw in title.lower()
                       for kw in concall_keywords):
                    concall_result = {"title": title, "url": url}
                    break
        except Exception:
            pass
        if concall_result:
    ...
    return {"articles": out[:5], "concall": concall_result}


@st.cache_data(ttl=86400, show_spinner=False)
def ai_summary_dated(company: str, exact_date: str, pct: float,
                     articles_json: str) -> str:
    """
    Generate AI summary using news from the EXACT event date.
    articles_json: JSON string of articles list (for cache compatibility).
    """
    if AI_PROVIDER == "none":
        return "Add GROQ_API_KEY to .env for AI summaries"

    articles = json.loads(articles_json)
    if not articles:
        return f"No news found for {exact_date} — search manually"
    ...
    prompt = (
        f"Equity research analyst. On {exact_date}, {company} stock "
        f"{direction} {abs(pct):.1f}% in a single month.\n\n"
        f"News from around that date:\n{ctx}\n\n"
        f"Write ONE precise sentence (≤25 words) explaining the cause.\n"
        f"Start with category: [Results], [Concall], [Macro], [M&A], "
        f"[Regulatory], [Sector], or [Other].\n"
        f"Be specific — name the actual catalyst if visible in the news."
    )
    try:
        if AI_PROVIDER == "groq":
            r = requests.post(
...
def enrich(events_df: pd.DataFrame, df_daily: pd.DataFrame,
           ticker: str, company: str) -> pd.DataFrame:
    """
    For each event month:
    1. Find the exact date of the biggest daily move in that month
    2. Search news for that EXACT date
    3. Search for concall link
    ...
    bar   = st.progress(0, text=f"Fetching dated news for {company}…")
    total = len(df)

    for i, (idx, row) in enumerate(df.iterrows()):
        # Find the exact date of largest daily move within that month
        month_daily = df_daily[
            df_daily.index.to_period("M") == idx.to_period("M")
        ]

        if not month_daily.empty:
            daily_pct = month_daily["Close"].pct_change().abs()
            if daily_pct.empty or daily_pct.isna().all():
                exact_dt = month_daily.index[-1]
            else:
                exact_dt = daily_pct.idxmax()   # day with biggest move
        else:
            exact_dt = idx

        exact_date_str = exact_dt.strftime("%d %b %Y")   # e.g. "19 Jan 2024"

        bar.progress(
            (i + 0.3) / total,
            text=f"Searching news for {exact_date_str} ({i+1}/{total})…"
        )

        # Fetch date-specific news + concall
        result   = get_news_for_date(company, exact_date_str)
        articles = result["articles"]
        concall  = result["concall"]

        bar.progress(
            (i + 0.7) / total,
            text=f"AI analysis: {exact_date_str} ({i+1}/{total})…"
        )

        # AI summary
        summary = ai_summary_dated(
            company, exact_date_str, row["pct"],
            json.dumps(articles)
        )

        summaries.append(summary)
        concalls.append(concall)
        news_lists.append(articles[:3])   # store top 3 for display
        time.sleep(0.2)
    ...
    df["concall"]    = concalls
    df["news"]       = news_lists
    # Store exact event date for display
    df["event_dates"] = [
        df_daily[df_daily.index.to_period("M") == idx.to_period("M")
                ]["Close"].pct_change().abs().idxmax()
        if not df_daily[df_daily.index.to_period("M") == idx.to_period("M")].empty
        else idx
        for idx in df.index
    ]
    return df
...
def price_chart(df_d, events, company, ticker, threshold=5.0):
    ...
    if not events.empty:
        for direction,color,sym,mult in [
            ...
            for idx,row in sub.iterrows():
                ...
                # Find exact date of last trading day in that month
                exact_date = sl.index[-1].strftime('%d %b %Y')
                month_label = idx.strftime('%b %Y')
                ht.append(
                    f"<b>{month_label}</b>  ·  Result date: {exact_date}<br>"
                    f"Monthly Move: <b style='color:{'#10B981' if pct>0 else '#EF4444'}'>"
                    f"{'+'if pct>0 else ''}{pct:.1f}%</b><br>"
                    f"Open: ₹{row['Open']:,.0f}  →  Close: ₹{row['Close']:,.0f}<br><br>"
                    f"<i style='color:#CBD5E1'>{summ}</i>")
            if xs:
                fig.add_trace(go.Scatter(x=xs,y=ys,mode="markers",
                    name=f"{'▲' if direction=='spike' else '▼'} ≥{threshold:.0f}% Monthly",
    ...
    fig.update_layout(
        ...
        title=dict(text=f"<b>{company}</b>  ·  {ticker}  ·  2Y Daily",
...
def event_table_html(events_df, df_daily=None):
    """
    Renders event log with:
    - Exact date of biggest daily move in that month
    - AI summary for that specific date
    - Clickable news article links
    - Concall link if found
    """
    if events_df.empty:
        return f"""<div style="background:{CARD};border:1px solid {BORDER};
                   border-radius:6px;padding:40px;text-align:center;color:{MUTED2}">
                   No events found at this threshold. Try lowering the slider.</div>"""
    rows = ""
    for idx, row in events_df.sort_index(ascending=False).iterrows():
        ...
        op      = row["Open"]
        cl      = row["Close"]
        summary = row.get("summary", "Enable AI Summaries in sidebar")
        sign    = "+" if pct > 0 else ""
        ...
        # Exact date — day with biggest move in that month
        event_date = "—"
        if "event_dates" in events_df.columns and pd.notna(row.get("event_dates")):
            try:
                event_date = pd.Timestamp(row["event_dates"]).strftime("%d %b %Y")
            except Exception:
                event_date = ms
            ...
            for n in news_list[:3]:
                ...
                if url and title:
                    news_html += (
                        ...
                        f"style='display:block;color:{BLUE2};font-size:10px;"
                        f"text-decoration:none;margin-top:3px;line-height:1.4;"
                        f"border-left:2px solid {BLUE2};padding-left:6px'>"
                        f"↗ {title}</a>"
                    )

        # Concall link
        concall     = row.get("concall")
        concall_html = ""
        if isinstance(concall, dict) and concall.get("url"):
            ctitle = concall.get("title","Concall / Investor Call")[:55]
            curl   = concall["url"]
            concall_html = (
                f"<a href='{curl}' target='_blank' "
                f"style='display:inline-block;margin-top:6px;"
                f"background:rgba(37,99,235,0.15);color:{BLUE2};"
                f"border:1px solid rgba(37,99,235,0.4);"
                f"padding:3px 10px;border-radius:4px;font-size:10px;"
                f"font-weight:600;text-decoration:none'>📞 {ctitle}</a>"
            )

        rows += f"""
        <tr style="background:{rb};border-bottom:1px solid {BORDER}">
            <td style="padding:12px 16px;white-space:nowrap;vertical-align:top">
                <div style="color:{TEXT};font-size:13px;font-weight:700">{ms}</div>
                <div style="color:{MUTED2};font-size:10px;margin-top:2px">
                    📅 {event_date}</div>
            </td>
            <td style="padding:12px 16px;white-space:nowrap;vertical-align:top">
                <span style="background:{bb};color:{tc};padding:3px 9px;
                             border-radius:3px;font-size:10px;font-weight:700">{lb}</span>
            </td>
            <td style="padding:12px 16px;white-space:nowrap;vertical-align:top;
                       color:{pc};font-size:15px;font-weight:800">{sign}{pct:.2f}%</td>
            <td style="padding:12px 16px;white-space:nowrap;vertical-align:top">
                <div style="color:{MUTED2};font-size:11px">Open</div>
                <div style="color:{TEXT};font-size:12px;font-weight:600">
                    ₹{op:,.0f}</div>
                <div style="color:{MUTED2};font-size:11px;margin-top:4px">Close</div>
                <div style="color:{TEXT};font-size:12px;font-weight:600">
                    ₹{cl:,.0f}</div>
            </td>
            <td style="padding:12px 18px;vertical-align:top;min-width:280px">
                <div style="color:#CBD5E1;font-size:12px;line-height:1.6;
                            font-style:italic">{summary}</div>
                {concall_html}
            </td>
            <td style="padding:12px 16px;vertical-align:top;min-width:280px">
                {news_html if news_html else
                 f"<span style='color:{MUTED};font-size:10px'>Enable AI Summaries for news links</span>"}
            </td>
        </tr>"""

    hdr = (f"padding:9px 16px;text-align:left;color:{MUTED};font-size:9px;"
    ...
    <div style="overflow-x:auto;overflow-y:auto;max-height:520px;
        ...
        <table style="width:100%;border-collapse:collapse;
            ...
            <thead><tr>
                <th style="{hdr};white-space:nowrap">Month / Date</th>
                <th style="{hdr};white-space:nowrap">Type</th>
                <th style="{hdr};white-space:nowrap">Move</th>
                <th style="{hdr};white-space:nowrap">Price</th>
                <th style="{hdr};min-width:280px">AI Summary + Concall</th>
                <th style="{hdr};min-width:280px">News Sources</th>
            </tr></thead>
            <tbody>{rows}</tbody>
        </table>
    </div>"""
...
def main():
    ...
    # Read threshold early — needed before any renders
    threshold = float(st.session_state.get("threshold_val", 5.0))
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-end;
        ...
        <div>
            ...
            <div style="font-size:12px;color:{MUTED2};margin-top:5px">
                40 stocks &nbsp;·&nbsp; 2-year history &nbsp;·&nbsp;
                Monthly move detection &nbsp;·&nbsp; AI catalyst summaries
            </div>
        </div>
    ...
    with st.spinner(f"Loading {company}…"):
        ...
        df_m   = tag_events(fetch_monthly(ticker), threshold)
    ...
    # ── Monthly move threshold slider ─────────────────────────────────────────
    threshold = st.sidebar.slider(
        "Monthly Move Threshold (%)",
        min_value=2.0, max_value=8.0,
        value=float(st.session_state.get("threshold_val", 5.0)),
        step=0.5, key="threshold_slider",
        help="Flag months where stock moved more than this % in either direction"
    )
    st.session_state["threshold_val"] = threshold
    st.sidebar.caption(f"Currently: ≥{threshold:.1f}% monthly move · 2Y history")
    ...
    cache_key  = f"{ticker}_{enable_ai}_{threshold}"

    if enable_ai and has_ai:
        if cache_key not in st.session_state:
            events = enrich(raw_events, df_d, ticker, company)
    ...
    c3.metric("▲ Monthly Spikes",n_spike, help=f"Months with ≥+{threshold:.0f}% move (5Y)")
    c4.metric("▼ Monthly Drops", n_drop,  help=f"Months with ≥−{threshold:.0f}% move (5Y)")
    ...
    # ─────────────── TAB 1 : PRICE ───────────────────────────────────────────
    with tab1:
        fig = price_chart(df_d, events, company, ticker, threshold)
        ...
        <div style="background:{PANEL};border:1px solid {BORDER};border-radius:6px;
            ...
            <div style="display:grid;grid-template-columns:repeat(3,1fr);
                ...
                <div><b style="color:{GREEN2}">▲ Green triangles</b>
                     <span style="color:{MUTED2}"> — months with ≥+{threshold:.0f}% move</span></div>
                <div><b style="color:{RED2}">▼ Red triangles</b>
                     <span style="color:{MUTED2}"> — months with ≥−{threshold:.0f}% drop</span></div>
    ...
    with tab4:
        ...
        <div style="display:flex;align-items:baseline;gap:12px;margin-bottom:14px">
            ...
            <span style="font-size:11px;color:{MUTED2}">
                {len(events)} events with ≥{threshold:.0f}% move in 5 years
